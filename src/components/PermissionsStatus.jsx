...
function statusMeta(status) {
    if (status === 'granted')    return { label: 'Granted',     color: '#10b981', bg: 'rgba(16,185,129,0.12)' };
    if (status === 'denied')     return { label: 'Denied',      color: '#ef4444', bg: 'rgba(239,68,68,0.1)' };
    if (status === 'prompt')     return { label: 'Not Asked',   color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' };
    if (status === 'requesting') return { label: 'Requesting…', color: '#6366f1', bg: 'rgba(99,102,241,0.1)' };
    if (status === 'unsupported')return { label: 'Unsupported', color: '#475569', bg: 'rgba(100,116,139,0.1)' };
    return                               { label: 'Unknown',    color: '#64748b', bg: 'rgba(100,116,139,0.08)' };
}
    ...
    return (
        <div className="glass-card" style={{ padding: 'var(--card-padding)', marginTop: 16 }}>
            <div style={{ borderBottom: '1px solid var(--glass-border)', paddingBottom: 12, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <h2 style={{ margin: 0, fontSize: '1rem', fontWeight: 700 }}>Browser Permissions</h2>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginLeft: 4 }}>Required for hardware sensor features</span>
            </div>
