                    ...
                    {showAddSensor && (
                        <div style={{ padding: '18px', background: 'rgba(99,102,241,0.05)', borderRadius: '10px', border: '1px solid rgba(99,102,241,0.3)' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                ...
                                <div><label style={labelStyle}>{t('sensor.plantId')}</label><input value={newSensor.plant_id} onChange={e => setNewSensor(p => ({...p, plant_id: e.target.value}))} placeholder={t('sensor.demo_plant_2')} style={inputStyle} title={t('sensors.plantIdWhereThisSensorTip')} /></div>
                                <div><label style={labelStyle}>{t('sensor.plantName')}</label><input value={newSensor.plant_name} onChange={e => setNewSensor(p => ({...p, plant_name: e.target.value}))} placeholder={t('sensor.demo_plant_2_name')} style={inputStyle} title={t('sensors.nameOfThePlantWhereTip')} /></div>
