 ...
 */
/**
 * Trier OS — Plant Setup & Configuration
 * ==========================================
 * Full CRUD for plant DNA: production model, production units, SKU catalog,
 * and plant calendar. All data is stored in trier_logistics.db keyed by PlantID.
 *
 * ENDPOINTS:
 *   GET    /api/plant-setup/config/:plantId         — Get plant configuration
 *   PUT    /api/plant-setup/config/:plantId         — Update plant configuration
 *   GET    /api/plant-setup/units/:plantId          — List production units
 *   POST   /api/plant-setup/units/:plantId          — Create production unit
 *   PUT    /api/plant-setup/units/:plantId/:id      — Update production unit
 *   DELETE /api/plant-setup/units/:plantId/:id      — Delete production unit
 *   GET    /api/plant-setup/skus/:plantId           — List SKU catalog
 *   POST   /api/plant-setup/skus/:plantId           — Create SKU entry
 *   PUT    /api/plant-setup/skus/:plantId/:id       — Update SKU entry
 *   DELETE /api/plant-setup/skus/:plantId/:id       — Delete SKU entry
 *   GET    /api/plant-setup/calendar/:plantId       — Get plant calendar
 *   PUT    /api/plant-setup/calendar/:plantId       — Update plant calendar
 *
 * KEY FEATURES:
 *   - Production model configuration (fluid-process, discrete, batch)
 *   - Production unit management with capacity planning
 *   - SKU catalog with yield rates and pack sizes
 *   - Plant calendar for shift and holiday scheduling
 *   - All settings scoped per PlantID in trier_logistics.db
 */
const express = require('express');
const router = express.Router();
const { db: logisticsDb } = require('../logistics_db');

// ΓöÇΓöÇ Table Init ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Safe column migrations ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const addCols = [
...
// ΓöÇΓöÇ Plant Configuration ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Production Units ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Products / SKU Catalog ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Plant Calendar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Seed Demo Data ΓÇö Dairy Processing Plant ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
router.post('/seed', (req, res) => {
    try {
        ...
        // ΓöÇΓöÇ Production Units ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        const units = [
            ...
            { name: 'Silo 1 ΓÇö Raw Milk',               type: 'Storage',    desc: '100,000 lb refrigerated vertical silo for raw whole milk. Primary holding before processing.',                     capacity: 100000, cunit: 'lbs',          sqft: 800,   crit: 'A', sort: 2  },
            { name: 'Silo 2 ΓÇö Raw Milk',               type: 'Storage',    desc: '100,000 lb refrigerated vertical silo. Backup and overflow raw milk capacity.',                                    capacity: 100000, cunit: 'lbs',          sqft: 800,   crit: 'B', sort: 3  },
            { name: 'Silo 3 ΓÇö Skim Milk',              type: 'Storage',    desc: '60,000 lb silo holding separated skim milk prior to standardization and HTST.',                                    capacity: 60000,  cunit: 'lbs',          sqft: 600,   crit: 'A', sort: 4  },
            ...
            { name: 'Buttermilk Culture Tanks',        type: 'Processing', desc: 'Two 3,000-gal jacketed culture tanks for cultured buttermilk. 18ΓÇô20 hr culture cycle at 72┬░F.',                  capacity: 6000,   cunit: 'gal/batch',    sqft: 800,   crit: 'B', sort: 12 },
            ...
            { name: 'Packaging Line 1 ΓÇö Gallons/HGL',  type: 'Packaging',  desc: 'High-speed HDPE gallon and half-gallon filler. All fat variants ΓÇö sequenced SkimΓåÆ1%ΓåÆ2%ΓåÆWhole. Air blow between changes.', capacity: 180, cunit: 'cases/min',  sqft: 3200,  crit: 'A', sort: 17 },
            { name: 'Packaging Line 2 ΓÇö Quarts/Pints', type: 'Packaging',  desc: 'Quart and pint filler. Fluid milk, chocolate milk, buttermilk, juice. Air blow required between product changes.', capacity: 120, cunit: 'cases/min',  sqft: 2400,  crit: 'A', sort: 18 },
            { name: 'Packaging Line 3 ΓÇö Small Sizes',  type: 'Packaging',  desc: '10oz, ┬╜ Pint (8oz), and 4oz small format filler. School lunch, foodservice, and club pack runs.',               capacity: 80,     cunit: 'cases/min',    sqft: 1800,  crit: 'B', sort: 19 },
            { name: 'Packaging Line 4 ΓÇö Juice/Drinks', type: 'Packaging',  desc: 'Dedicated juice and fruit drink filling line. HDPE gallon through 8oz. Isolated from milk circuit.',             capacity: 60,     cunit: 'cases/min',    sqft: 1600,  crit: 'B', sort: 20 },
            { name: 'Cold Storage ΓÇö Room A',           type: 'Storage',    desc: 'Primary finished goods cold storage at 34┬░F. Fluid milk, 48,000-case capacity. Direct truck dock.',              capacity: 48000,  cunit: 'cases',        sqft: 12000, crit: 'A', sort: 21 },
            { name: 'Cold Storage ΓÇö Room B',           type: 'Storage',    desc: 'Secondary cold storage at 34┬░F. Juice, drinks, and overflow milk. 32,000-case capacity.',                        capacity: 32000,  cunit: 'cases',        sqft: 8000,  crit: 'A', sort: 22 },
            ...
            { name: 'Refrigeration ΓÇö Ammonia System',  type: 'Utilities',  desc: 'Central ammonia refrigeration plant. Feeds silo cooling jackets, cold storage rooms, and process chillers.',     capacity: 800,    cunit: 'tons',         sqft: 2400,  crit: 'A', sort: 25 },
        ...
        // ΓöÇΓöÇ SKU Catalog ΓÇö Full Dairy Matrix ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        //
        // Production run sequence (ascending butterfat minimizes waste):
        //   Skim (0%) ΓåÆ 1% ΓåÆ 2% ΓåÆ Whole (3.25%) ΓåÆ Choc 2% ΓåÆ Choc Whole
        //   Air blow required between EVERY fat-content change.
        //   First 3ΓÇô5 gallons after blow = "blend" diverted to animal feed.
        ...
        const FLUID_PRODUCTS = [
            { code: 'SKIM',  name: 'Skim Milk',           fat: 0.0,  seq: 1,  changeover: 'none',     sizes: ['GAL','HGL','QT','PT','HP','4OZ'], baseQty: 1200, hPct: 0.50, notes: 'Run first ΓÇö zero butterfat. No air blow needed at start of run.' },
        ...
        const CHOC_PRODUCTS = [
            ...
            { code: 'CHOCW',  name: 'Chocolate Whole Milk', fat: 3.25, seq: 6,  changeover: 'air-blow',    sizes: ['GAL','HGL','QT','PT','HP','4OZ'], baseQty: 480, hPct: 0.30, labels: ['LA','LB','LC','LD'], notes: 'After choc 2%. Air blow only ΓÇö same cocoa system. Highest fat chocolate.' },
        ];

        const OTHER_PRODUCTS = [
            { code: 'BTRMILK', name: 'Cultured Buttermilk', fat: 0.5,  seq: 7,  changeover: 'full-cip', sizes: ['GAL','QT','PT'], baseQty: 320, hPct: 0.20, labels: ['LA','LB','LC'], notes: 'Separate cultured run on dedicated line. Full CIP before and after. 18ΓÇô20 hr culture cycle.' },
            { code: 'OJ',      name: 'Orange Juice',         fat: 0,    seq: 8,  changeover: 'full-cip', sizes: ['GAL','HGL','QT','HP'], baseQty: 280, hPct: 0.25, labels: ['LA','LB'], notes: 'Juice line ΓÇö fully isolated from milk circuits. No cross-contamination risk.' },
            ...
            for (const prod of products) {
                ...
                for (const lbl of labels) {
                    for (const sizeCode of prod.sizes) {
                        ...
                        const name = `${prod.name} ΓÇö ${sz.label} [${lbl.name}]`;
        ...
        genSKUs(OTHER_PRODUCTS, null, false); // family set in loop below ΓÇö fix
        ...
        // ΓöÇΓöÇ Non-matrix products (cream, butter, cheese, ingredients) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        const extras = [
            { sku: 'HVYCREAM-PT',   name: 'Heavy Whipping Cream ΓÇö Pint',        family: 'Cream',       fat: 36,  seq: 20, co: 'none', qty: 800,  hqty: 250, uom: 'cases', label: 'Label A', sz: 'PT',  oz: 16 },
            { sku: 'HVYCREAM-QT',   name: 'Heavy Whipping Cream ΓÇö Quart',       family: 'Cream',       fat: 36,  seq: 20, co: 'none', qty: 400,  hqty: 120, uom: 'cases', label: 'Label A', sz: 'QT',  oz: 32 },
            { sku: 'HALFHALF-PT',   name: 'Half & Half ΓÇö Pint',                 family: 'Cream',       fat: 12,  seq: 21, co: 'none', qty: 640,  hqty: 200, uom: 'cases', label: 'Label A', sz: 'PT',  oz: 16 },
            { sku: 'HALFHALF-QT',   name: 'Half & Half ΓÇö Quart',                family: 'Cream',       fat: 12,  seq: 21, co: 'none', qty: 320,  hqty: 100, uom: 'cases', label: 'Label A', sz: 'QT',  oz: 32 },
            { sku: 'SRCREAM-16OZ',  name: 'Sour Cream ΓÇö 16 oz',                 family: 'Cream',       fat: 20,  seq: 22, co: 'none', qty: 320,  hqty: 90,  uom: 'cases', label: 'Label A', sz: 'HP',  oz: 16 },
            { sku: 'BUTTER-SALT',   name: 'Salted Butter ΓÇö 1 lb Print',         family: 'Butter',      fat: 80,  seq: 30, co: 'none', qty: 1200, hqty: 300, uom: 'cases', label: 'Label A', sz: 'LB',  oz: 16 },
            { sku: 'BUTTER-UNSALT', name: 'Unsalted Butter ΓÇö 1 lb Print',       family: 'Butter',      fat: 80,  seq: 31, co: 'none', qty: 900,  hqty: 250, uom: 'cases', label: 'Label A', sz: 'LB',  oz: 16 },
            { sku: 'CHEDDAR-8OZ',   name: 'Shredded Cheddar ΓÇö 8 oz',            family: 'Cheese',      fat: 30,  seq: 40, co: 'none', qty: 600,  hqty: 180, uom: 'cases', label: 'Label A', sz: '8OZ', oz: 8  },
            { sku: 'MOZZ-8OZ',      name: 'Shredded Mozzarella ΓÇö 8 oz',         family: 'Cheese',      fat: 20,  seq: 41, co: 'none', qty: 480,  hqty: 150, uom: 'cases', label: 'Label A', sz: '8OZ', oz: 8  },
            { sku: 'COTTAGE-16OZ',  name: 'Cottage Cheese ΓÇö 16 oz',             family: 'Cheese',      fat: 4,   seq: 42, co: 'none', qty: 360,  hqty: 100, uom: 'cases', label: 'Label A', sz: '16OZ',oz: 16 },
            { sku: 'CREAMCHS-8OZ',  name: 'Cream Cheese ΓÇö 8 oz Block',          family: 'Cheese',      fat: 33,  seq: 43, co: 'none', qty: 280,  hqty: 80,  uom: 'cases', label: 'Label A', sz: '8OZ', oz: 8  },
            { sku: 'WPC34-5LB',     name: 'Whey Protein Concentrate 34% ΓÇö 5 lb',family: 'Ingredients', fat: 4,   seq: 50, co: 'none', qty: 120,  hqty: 30,  uom: 'bags',  label: 'Label A', sz: '5LB', oz: 80 },
        ];
        ...
        // ΓöÇΓöÇ Plant Calendar ΓÇö 2026 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        const calEvents = [
            ...
            { date: '2026-07-03', type: 'shutdown',            label: 'Independence Day (observed) ΓÇö Shutdown', pct: 0   },
            ...
            { date: '2026-11-11', type: 'reduced-production',  label: "Veterans' Day ΓÇö Reduced Crew",           pct: 50  },
            { date: '2026-11-25', type: 'reduced-production',  label: 'Day Before Thanksgiving ΓÇö Early Release',pct: 75  },
            { date: '2026-11-26', type: 'federal-holiday',     label: 'Thanksgiving Day',                       pct: 0   },
            { date: '2026-11-27', type: 'shutdown',            label: 'Day After Thanksgiving ΓÇö Plant Shutdown',pct: 0   },
            { date: '2026-12-24', type: 'shutdown',            label: 'Christmas Eve ΓÇö Plant Shutdown',         pct: 0   },
            { date: '2026-12-25', type: 'federal-holiday',     label: 'Christmas Day',                          pct: 0   },
            { date: '2026-12-26', type: 'shutdown',            label: 'Day After Christmas ΓÇö Shutdown',         pct: 0   },
            { date: '2026-12-31', type: 'shutdown',            label: "New Year's Eve ΓÇö Early Shutdown",        pct: 20  },
        ];
...
// ΓöÇΓöÇ Summary stats ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ API Integration Configuration ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// Test connection ΓÇö performs a lightweight reachability check for known providers
router.post('/integrations/test', async (req, res) => {
    ...
    // Cannot test ΓÇö no host/endpoint configured
    if (!host) return res.json({ ok: false, message: 'No host or endpoint configured ΓÇö fill in connection details first.' });
