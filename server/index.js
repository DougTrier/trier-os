...
// â”€â”€ License Activation System â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const license = require('./license');
let _licenseValid = false;
let _licenseInfo = null;

// DEMO_MODE: bypass license for portable demo builds
const _demoMode = process.env.DEMO_MODE === 'true';
if (_demoMode) {
    _licenseValid = true;
    _licenseInfo = { valid: true, machineId: 'DEMO', expires: 'DEMO', daysLeft: Infinity, licenseType: 'Demo Mode' };
    console.log('[LICENSE] ðŸŽ¯ DEMO MODE â€” License check bypassed');
} else {
    // Check license on boot
    const _licenseCheck = license.checkLicense(_resolvedDataDir);
    _licenseValid = _licenseCheck.valid;
    _licenseInfo = _licenseCheck;
    if (_licenseValid) {
        const expiryMsg = _licenseCheck.expires === 'PERPETUAL' 
            ? '(Perpetual)' 
            : `(Expires: ${_licenseCheck.expires}, ${_licenseCheck.daysLeft} days left)`;
        console.log(`[LICENSE] âœ… Valid license for machine ${_licenseCheck.machineId} ${expiryMsg}`);
    } else if (_licenseCheck.expired) {
        console.log(`[LICENSE] â° License EXPIRED on ${_licenseCheck.expires}. Machine ID: ${_licenseCheck.machineId}`);
    } else {
        console.log(`[LICENSE] âš ï¸  No valid license found. Machine ID: ${_licenseCheck.machineId}`);
        console.log(`[LICENSE] Server will show activation page until a valid key is entered.`);
    }
}

// Re-check expiry every hour (catches mid-run expiration)
setInterval(() => {
    const check = license.checkLicense(_resolvedDataDir);
    if (_licenseValid && !check.valid) {
        console.log(`[LICENSE] â° License has expired! Server will require re-activation.`);
    }
    _licenseValid = check.valid;
    _licenseInfo = check;
}, 60 * 60 * 1000);

// License activation endpoint (always available)
app.post('/api/license/activate', express.urlencoded({ extended: true }), (req, res) => {
    const key = (req.body.licenseKey || '').trim();
    const machineId = license.getMachineId();
    
    // Try all possible expiry combos: perpetual first, then try extracting expiry from key format
    // The customer pastes just the key â€” we try PERPETUAL first
    if (license.validateLicenseKey(key, machineId, 'PERPETUAL')) {
        license.saveLicense(_resolvedDataDir, key, 'PERPETUAL');
        _licenseValid = true;
        _licenseInfo = license.checkLicense(_resolvedDataDir);
        console.log(`[LICENSE] âœ… Perpetual license activated!`);
        res.redirect('/');
        return;
    }
    
    // Try with specific expiry dates â€” check if an expiry was submitted via hidden field or query
    const expiry = (req.body.expiry || '').trim().toUpperCase();
    if (expiry && license.validateLicenseKey(key, machineId, expiry)) {
        license.saveLicense(_resolvedDataDir, key, expiry);
        _licenseValid = true;
        _licenseInfo = license.checkLicense(_resolvedDataDir);
        console.log(`[LICENSE] âœ… License activated! Expires: ${expiry}`);
        res.redirect('/');
        return;
    }
    
    // Try brute-force matching expiry dates (Â±2 years range) in case customer forgot expiry field
    const today = new Date();
    for (let d = 1; d <= 1826; d++) {
        const tryDate = new Date(today);
        tryDate.setDate(tryDate.getDate() + d);
        const tryStr = tryDate.toISOString().split('T')[0];
        if (license.validateLicenseKey(key, machineId, tryStr)) {
            license.saveLicense(_resolvedDataDir, key, tryStr);
            _licenseValid = true;
            _licenseInfo = license.checkLicense(_resolvedDataDir);
            console.log(`[LICENSE] âœ… License activated! Expires: ${tryStr}`);
            res.redirect('/');
            return;
        }
    }
    
    const errInfo = _licenseInfo && _licenseInfo.expired ? _licenseInfo : null;
    res.status(200).send(license.getActivationPageHTML(machineId, 'Invalid license key. Please check and try again.', errInfo));
});

// License status API (for programmatic checks)
app.get('/api/license/status', (req, res) => {
    ...
    res.json({ 
        ...
        licenseType: check.licenseType || null
    });
});

// License gate middleware â€” blocks everything except assets when unlicensed
app.use((req, res, next) => {
    // Always allow: static assets, license endpoints, favicon
    if (_licenseValid || 
    ...
    // Always allow auth and enrollment endpoints — needed before and after licensing
    if (req.path.startsWith('/api/auth/') || req.path.startsWith('/api/enrollment/')) {
        req._creatorBypass = true;
        return next();
    }
    
    // Show expired page or activation page
    const machineId = license.getMachineId();
    const expiredInfo = _licenseInfo && _licenseInfo.expired ? _licenseInfo : null;
    res.status(200).send(license.getActivationPageHTML(machineId, '', expiredInfo));
});
